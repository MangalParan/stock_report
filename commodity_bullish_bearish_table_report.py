    ...
    <div class="container">
        <header>
            <div>
                ...
                <p style="font-size: 0.85em; margin-top: 8px;">Generated: {datetime.now(IST).strftime('%d-%b-%Y %I:%M:%S %p IST')}</p>
                <p style="font-size: 0.8em; margin-top: 4px; opacity: 0.9;">AI Generated by Mangal</p>
            </div>
...
"""
    ...
    html_content += f"""            </tbody>
        ...
        <div class="timestamp">
            Last updated: {report_data['timestamp']} | AI Generated by Mangal
        </div>
    </div>
    ...
    <script>
        ...
        async function waitForNewRun(token, triggerTime) {{
            ...
            for (let i = 0; i < 30; i++) {{
                ...
                document.getElementById('progressStatus').textContent = 'Waiting for workflow to start...';
            }}
        ...
        async function refreshData() {{
            ...
            try {{
                ...
                document.getElementById('progressStatus').textContent = 'Workflow dispatched. Waiting for run to start...';
                document.getElementById('progressFill').style.width = '5%';
                document.getElementById('progressPercent').textContent = '5%';

                const runId = await waitForNewRun(token, triggerTime);
                document.getElementById('progressStatus').textContent = 'Workflow started. Tracking progress...';
